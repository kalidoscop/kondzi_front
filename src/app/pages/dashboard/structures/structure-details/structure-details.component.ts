...
export class StructureDetailsComponent implements OnInit{
  ...
  ngOnInit(): void {
    ...
    // console.log(id);
    if (id) {
      this.structureService.getStructur(id).subscribe((res)=> {
        console.log(res)
        this.structureForm =new FormGroup({
          name: new FormControl(`${res.name}`, Validators.required),
          domaine: new FormControl(`${res.domaine}`, Validators.required),
          managerName: new FormControl(`${res.manager_name}`, Validators.required),
          managerTitle: new FormControl(`${res.manager_title}`, Validators.required),
          tel: new FormControl(`${res.tel}`, Validators.required),
          email: new FormControl(`${res.email}`, [Validators.required, Validators.email]),
          openingHours: new FormControl(`${res.opening_hours}`, Validators.required),
